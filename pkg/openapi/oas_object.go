...
// OasObjectByName takes parent and a name under which object can be found and creates a wrapper over OpenAPI object.
// The forceCreate argument controls whether not-existing underlying object should be constructed and initialized with zero value.
func OasObjectByName(parent interface{}, name string, forceCreate bool) (OasObject, error) {
	obj := &OasObject{
		parent: parent,
		name:   name,
	}
	...
	err := obj.parse()
	if err != nil {
		if !forceCreate && !isNotExisitngObject(err) {
			return *obj, err
		}
...
// OasObjectByIdx takes parent and an index under which object can be found
func OasObjectByIdx(parent interface{}, idx int) (OasObject, error) {
	obj := &OasObject{
		parent: parent,
		idx:    idx,
	}
...
// Create forces the underlying OasObject to be replaced with zero value.
...
// The operation is destructive to the underlying object if not zeroed.
func (o *OasObject) Create() error {
...
func (o OasObject) references() ([]reference, error) {
	...
	case reflect.Ptr:
		...
		if refPath != "" {
			...
			for _, field := range fields {
				obj, err := OasObjectByName(value.Interface(), field, false)
				if err != nil {
					return allRefs, err
				}
	...
	case reflect.Map:
		...
		for _, key := range keys {
			obj, err := OasObjectByName(value.Interface(), key, false)
			if err != nil {
				return allRefs, err
			}
	...
	case reflect.Slice:
		...
		for _, idx := range indexes {
			obj, err := OasObjectByIdx(value.Interface(), idx)
			if err != nil {
				return allRefs, err
			}
...
func isNotExisitngObject(err error) bool {
	return errors.Is(err, ErrFieldWithNameUnusable) || errors.Is(err, ErrNoValueWithKey)
}
